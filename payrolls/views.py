...
def create_payment_batch_view(request, period_id):
    ...
    return redirect("payroll:execute_payment_batch", batch.id)
...
@login_required
def payroll_batch_dashboard(request):
    user = request.user
    ...
    if user.role in ["ADMIN", "BURSAR"]:
        ...
        return render(request, "payroll/admin/payroll_dashboard.html", context)
...
def payroll_period_detail(request, period_id):
    ...
    records = PayrollRecord.objects.filter(
        payroll_period=period
    ).select_related("payee").order_by('-created_at')
...
def approve_payroll_period(request, period_id):
    ...
    # Bulk approve all records for this period so they can be paid
    from payroll.models import PayrollApproval, PayrollRecord
    
    # Get IDs of records in this period
    record_ids = PayrollRecord.objects.filter(payroll_period=period).values_list('id', flat=True)
    
    # Ensure Approval objects exist (for legacy records)
    existing_approvals = PayrollApproval.objects.filter(payroll_record__in=record_ids).values_list('payroll_record_id', flat=True)
    missing_ids = set(record_ids) - set(existing_approvals)
    
    if missing_ids:
        PayrollApproval.objects.bulk_create([
            PayrollApproval(payroll_record_id=rid) for rid in missing_ids
        ])

    # Update approvals
    # Note: If user is ADMIN, we grant Admin approval.
    # If BURSAR, we might only grant Bursar approval, but 'is_approved' on period usually implies full approval.
    # Given the previous context, let's assume this action fully approves the period for payment.
    
    approvals = PayrollApproval.objects.filter(payroll_record__payroll_period=period)
    ...
    if request.user.role == "BURSAR":
         update_data['bursar_approved_by'] = request.user
         update_data['bursar_approved_at'] = timestamp
    
    if request.user.role == "ADMIN":
         # Admin approval usually supersedes, but we should probably ensure Bursar is set too or irrelevant?
         # For now, let's set Admin approval.
         update_data['admin_approved_by'] = request.user
         update_data['admin_approved_at'] = timestamp
         
         # Optional: Auto-fill bursar approval if empty? 
         # Or assume Admin can override. 
         # batch_service checks `approval__admin_approved_at__isnull=False`.
    
    approvals.update(**update_data)
...
def admin_finance_dashboard(request):
    ...
    status = PaymentBatch.is_processed
